...
interface SearchPageProps {
  ...
  isAuthenticated: boolean    // ← add
}

type SearchResult = { symbol: string; name: string | null; exchange?: string | null; currency?: string | null }

export default function SearchPage({ onTickerNav, initialQuery = '', tickers, refreshData, isAuthenticated }: SearchPageProps) {
  const [query, setQuery] = useState(initialQuery)
  const [results, setResults] = useState<SearchResult[]>([])
  const [loading, setLoading] = useState(false)
  const abortRef = useRef<AbortController | null>(null)
  const { openAuthModal } = useAuthModal()    // ← add
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          console.error('search-symbol failed', res.status, await res.text())
          setResults([])
        } else {
        ...
        if ((err as any).name !== 'AbortError') console.error('search-symbol fetch error', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSelect = async (sym: string) => {
    // ← gate here — auth check before doing anything
    if (!isAuthenticated) {
      openAuthModal(`Sign in to open the full chart, news, and AI summaries for ${sym}.`)
      return
    }
    try {
      const res = await fetch(`/api/sync-ticker/${encodeURIComponent(sym)}`)
      if (!res.ok) {
        console.error('sync ticker failed', res.status, await res.text())
        return
      }
      await refreshData()
      onTickerNav(sym)
  ...
  return (
    <div className="p-6">
      ...
      <div className="relative mb-2">
        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--text3)] pointer-events-none">🔍</span>
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="e.g. MSFT, Apple, semiconductors..."
          className="w-full bg-[var(--bg3)] border border-[var(--border)] rounded-lg py-2.5 pl-10 pr-3 text-[15px] text-[var(--text)] placeholder-[var(--text3)] outline-none focus:border-[var(--accent)] transition-colors"
          autoFocus
        />
      </div>
          ...
          <>
            {results.map((row) => (
              ...
              >
                ...
                {!isAuthenticated && (
                  <span className="text-[10px] font-mono-custom text-[var(--text3)] border border-[var(--border)] rounded px-1.5 py-0.5">
                    Sign in to open
                  </span>
                )}
              </div>
              ...
              localMatches.map(([sym, t]) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <div className="font-mono-custom text-[14px] font-medium text-[var(--text)]">{t.price}</div>
                    <div className={`font-mono-custom text-[11px] ${t.dir === 'up' ? 'text-[var(--accentg)]' : 'text-[var(--accentr)]'}`}>
                      {t.chg}
                    </div>
                  </div>
