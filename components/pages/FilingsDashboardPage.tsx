...
export default function FilingsDashboardPage({ onTickerNav, isAuthenticated }: FilingsDashboardPageProps) {
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      openAuthModal('Sign in to access the Filings Dashboard and browse SEC filings with AI-generated digests.')
      return
    }
  ...
  if (!isAuthenticated) {
    return (
      <div className="p-6">
        ...
        <button
          onClick={() => openAuthModal('Sign in to access the Filings Dashboard and browse SEC filings with AI-generated digests.')}
    ...
    return (
      <div className="p-6">
        <div className="text-center py-12">
          <h1 className="text-2xl font-serif-custom italic text-[var(--text)] mb-2">Filings Dashboard</h1>
  ...
  if (error || !stats) {
    return (
      <div className="p-6">
        <div className="text-center py-12">
          <h1 className="text-2xl font-serif-custom italic text-[var(--text)] mb-2">Filings Dashboard</h1>
          <p className="text-[var(--accentr)]">{error ?? 'No data available'}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[var(--text3)] text-sm font-mono-custom uppercase tracking-[1px]">Total Filings</p>
              <p className="text-4xl font-serif-custom italic font-bold text-[var(--text)] mt-2">{stats.totalFilings}</p>
            </div>
            <div className="w-16 h-16 bg-[var(--accent)]/10 rounded-lg flex items-center justify-center text-3xl">📊</div>
          </div>
        </div>
        <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[var(--text3)] text-sm font-mono-custom uppercase tracking-[1px]">Imported Today</p>
              <p className="text-4xl font-serif-custom italic font-bold text-[var(--accentg)] mt-2">{stats.todayFilings}</p>
            </div>
            <div className="w-16 h-16 bg-[#10b981]/10 rounded-lg flex items-center justify-center text-3xl">✨</div>
          </div>
        </div>
      </div>
              ...
              {stats.latestCompanies.map((company, idx) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="text-xl font-serif-custom italic font-bold text-[var(--accent)]">{company.ticker}</div>
                    <div className="text-sm text-[var(--text3)]">{company.count} filing{company.count !== 1 ? 's' : ''}</div>
                  </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--border)] bg-[var(--bg3)]">
                    {['Ticker', 'Type', 'Date', 'Summary', 'Action'].map((h) => (
                      <th key={h} className={`px-6 py-3 text-xs font-mono-custom uppercase text-[var(--text3)] tracking-[1px] ${h === 'Action' ? 'text-center' : 'text-left'}`}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {stats.allFilings.map((filing, idx) => (
                    <tr key={idx} className="border-b border-[var(--border)] hover:bg-[var(--bg3)] transition-colors">
                      <td className="px-6 py-4 text-sm font-serif-custom italic font-bold text-[var(--accent)]">{filing.ticker}</td>
                      <td className="px-6 py-4 text-sm text-[var(--text)]">{filing.report_type}</td>
                      <td className="px-6 py-4 text-sm text-[var(--text2)]">
                        {new Date(filing.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-[var(--text2)] line-clamp-2">{filing.level1 || 'No summary available'}</p>
                      </td>
      ...
      {selectedFiling && (
        <FilingSummaryViewer filing={selectedFiling} onClose={() => setSelectedFiling(null)} />
      )}
    </div>
  )
}